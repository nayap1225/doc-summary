...
<template>
  <div class="flex flex-col space-y-4">
    <!-- Actions Bar -->
    <div v-if="store.files.length > 0" class="flex justify-between items-center border-b border-gray-100 pb-4">
      <h3 class="text-lg font-semibold text-gray-700 flex items-center gap-2">
        ...
        <div class="flex items-center ml-2 border-l border-gray-200 pl-2 gap-1">
          ...
          <button 
            ...
            :title="expandedItems.size > 0 ? '모두 접기' : '모두 펼쳐보기'"
          >
        ...
        >
          ...
          <div class="p-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
               ...
               <div v-if="item.status === 'idle'" class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500">
                  <Circle class="w-5 h-5" />
               </div>
               <div v-else-if="item.status === 'parsing'" class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-500">
                  <Loader2 class="w-5 h-5 animate-spin" />
               </div>
               <div v-else-if="item.status === 'done'" class="w-10 h-10 rounded-full bg-green-50 flex items-center justify-center text-green-600">
                  <CheckCircle2 class="w-5 h-5" />
               </div>
               <div v-else class="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center text-red-500">
                  <AlertCircle class="w-5 h-5" />
               </div>

               <div>
                 <h4 class="font-medium text-gray-800">{{ item.file.name }}</h4>
                 <p class="text-xs text-gray-400">{{ (item.file.size / 1024).toFixed(1) }} KB</p>
               </div>
            </div>
            ...
            <div class="flex gap-2 items-center">
              ...
              <span v-if="item.status === 'done'" class="flex items-center gap-2 text-sm text-green-600 font-medium">
                <span 
                  class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"
                  :class="item.resultType === 'summary' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'"
                >
          ...
          <!-- Summary Result -->
          <!-- Summary Result -->
          <div v-if="item.summary && item.status !== 'error'" class="bg-gray-50 border-t border-gray-100 p-4">
             <div class="cursor-pointer" @click="toggleExpand(item.id)">
               <div class="flex items-center justify-between text-sm font-medium text-gray-700 hover:text-blue-600 select-none">
                 <span>펼쳐보기</span>
                 <ChevronDown 
