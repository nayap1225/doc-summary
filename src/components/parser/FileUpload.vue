...
const validateFiles = (fileList: FileList): File[] => {
  const allowedExtensions = ['txt', 'docx', 'pdf', 'hwp', 'mp3', 'wav', 'm4a'];
  ...
  if (invalidFiles.length > 0) {
    ...
    alertStore.open('지원하지 않는 파일', `다음 파일은 업로드할 수 없습니다:\n${invalidFiles.join('\n')}\n(지원 형식: TXT, DOCX, PDF, HWP, MP3, WAV, M4A)`, 'error');
  }
...
<template>
  <div class="border-2 border-dashed rounded-lg py-16 px-4 text-center transition-colors cursor-pointer" :class="isDragging ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600 hover:border-blue-400 bg-white dark:bg-gray-800'" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="onDrop" @click="fileInput?.click()">
    ...
    <div class="flex flex-col items-center justify-center gap-2 text-gray-500 dark:text-gray-400">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-500 mt-1">
        ...
        지원하는 음성 포멧: <span class="font-semibold text-indigo-600 dark:text-indigo-400">MP3, WAV</span>
      </p>
    </div>
  </div>
</template>
